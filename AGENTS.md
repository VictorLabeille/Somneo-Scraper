...
| `collector/` — le serveur FastAPI, « le collecteur » du cadrage et des notes | **À écrire** | C'est le travail en cours. Tout nouveau code applicatif va là. |
...
Le code applicatif va dans **`collector/`** (arrêté le 2026-09-12) ; sa structure est au §2 du
plan technique. Deux règles tiennent la connexion unique au réveil **par construction** :
**seul `gateway.py` importe `pysomneo`** — aucun autre module ne parle à l'appareil — et
**uvicorn tourne avec un seul worker**, un second processus ouvrant une seconde connexion.
...
- **Exception à « pysomneo pour tout dialogue » : les profils d'alarme s'écrivent par un `PUT
  ...
  catalogue. `sndss` (départ en douceur) est mesuré (`probes/sndss.py`, 2026-09-13) : inscriptible,
  non borné, sens physique inconnu — relayé en numéro brut relu-confirmé, à ne pas surfacer côté
  app tant que la correspondance n'est pas établie (comme `lgtds`).
...
- **Le poste de dev ne peut pas découvrir le réveil** : WSL2 est derrière un NAT que le
  ...
  cette impasse.
- **`ensurepip` manque au Python du poste de dev comme à celui de la carte**, donc `python3 -m
  venv` seul échoue. Créer le venv **`--without-pip`** puis l'amorcer par `get-pip.py` — c'est
  déjà ce que fait `collector/deploy/install.sh`, s'en inspirer plutôt que conclure qu'on ne peut
  rien installer. La suite de `collector/` se lance ainsi depuis le poste.
